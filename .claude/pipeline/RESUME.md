...
**開工第一件事是決定「四面」怎麼定義。** 目前的牆是任意 `WallSegment`,不是固定
四面的盒子:使用者可以畫零面、三面、八面。兩條路 ——

- **依方位分組**(法線朝北/南/東/西):UI 恆定四組,貼近競品與使用者的直覺,
  但斜牆要決定歸到哪一組。
- **逐面牆各自設定**:貼合現有資料模型,但 UI 會隨牆數長大,八面牆就是八組。

沒有牆時不得出現空選單(條件 6),柱子跟哪一組要明確定義(條件 7,第二輪已定
「柱子不給獨立選項」)。破壞驗證是「讓兩組共用同一個材質物件 → 條件 2 應失敗」,
所以**探針必須從 GPU 讀回實際材質**,不是讀選單的值。
