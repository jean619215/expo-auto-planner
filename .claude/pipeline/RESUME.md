...
> 最後更新:2026-08-26(T1–T8 完成,只剩 T9)。
...
| 第三輪 T1–T8(範圍 / 目錄 / 繪製 / AI / 程序化幾何 / 目錄擴充 / 目錄頁 UI / 報價小計)| ✅ 完成 |
| 第三輪 T9(四面牆各自貼圖)| ⬜ 待開工,**範圍已定案**(見下)|
...
驗收條件在 story 第五節(8 項)。範圍已於 2026-08-26 由使用者定案:
**只做牆面分組貼圖** —— 不做家具換材質、不做招牌上傳圖(兩者都另立 story)。

**「四面」的定義已定案(2026-08-26):逐面牆各自設定。** 畫面上列「牆 1、牆 2、
牆 3……」,點哪面改哪面;不是依方位分成恆定四組。材質設定跟著 `WallSegment` 走,
牆刪掉重畫設定就消失 —— 那是誠實的行為,不要想辦法救回來。沒有牆時清單是空的,
分組 UI 不出現(條件 6 自然成立)。

**還剩一個細節要拍板:柱子跟哪一組(條件 7)。** 逐面牆之後柱子沒有「所屬牆」,
而第二輪已定「柱子不給獨立選項」。提案:保留一份**全域預設牆面材質**,新畫的牆
與柱子都用它,個別牆可覆寫 —— 柱子因此有明確歸屬,一面牆都沒畫時也還有東西可設。
實作前先跟使用者確認這一句。

破壞驗證是「讓兩組共用同一個材質物件 → 條件 2 應失敗」,所以**探針必須從 GPU
讀回實際材質**,不是讀選單的值。

**第三輪的收尾方式也定案了**:T9 完成後才一次合回 `master`。PR #15 先合進
`docs/venue-catalog-and-quote`,T9 另開 PR,整輪做完再一起上 master。
