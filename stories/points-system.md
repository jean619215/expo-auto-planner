...
- 在已登入狀態下,當使用者查看商店頁,則能看到目前點數餘額與交易記錄;Header 有「點數商店」連結(登入時顯示)。
...
- [x] [FRONTEND] 商店頁 /shop(三種點數方案 + 結帳流程 + 餘額/交易記錄)+ mock 金流結帳頁 /shop/mock-checkout + Header「點數商店」連結,Playwright 驗收(points-shop.spec.ts,page object ShopPage.ts)
